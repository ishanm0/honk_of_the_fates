...
void QEI_IRQ() {
    ...
    if (buffer == 4) { // if buffer is 4, then decrement rotary count
    // buffer is size of 4 because the rotary encoder has to be changed 4 times to be confident of a new count
        rotary_count--;
        buffer = 0;
    } else if (buffer == -4) { // if buffer is -4, then increment rotary count
        rotary_count++;
        buffer = 0;
    }
    ...
    if (rotary_count == 8) { // if rotary count is 24, then reset to 0 since full rotation
        rotary_count = 0;
