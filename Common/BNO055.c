...
int8_t BNO055_Init(void) {
    ...
    DelayMicros(1000000); // delaying to ensure that successive programs do not glitch the sensor
    ...
    DelayMicros(25000); // delay between changing op modes > 19 msec
    byteReturn = I2C_WriteReg(BNO055_ADDRESS_A, BNO055_PWR_MODE_ADDR, POWER_MODE_NORMAL);
...
int main(void) {
   ...
   /*
    unsigned char byteReturn = I2C_ReadRegister(BNO055_ADDRESS_A, BNO055_OPR_MODE_ADDR);
